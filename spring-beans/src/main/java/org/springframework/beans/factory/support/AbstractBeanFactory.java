...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		if (sharedInstance != null && args == null) { //如果获取到单例bean，则走下面代码
			...
			try {
				...
				String[] dependsOn = mbd.getDependsOn(); // 获取bean的依赖对象，也就是这些bean 需要优先创建。
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						...
						try {
							getBean(dep); // 优先创建依赖的Bean，也就是在创建当前bean之前需要优先创建的Bean
						}
				...
				if (mbd.isSingleton()) {
					...
					// 如果Bean是FactoryBean,需要从FactoryBean中获取真正的Bean
					bean = getObjectForBeanInstance(sharedInstance, name, beanName, mbd);
				}

				else if (mbd.isPrototype()) { // 不允许发生循环依赖
					...
					try {
						Object scopedInstance = scope.get(beanName, () -> {
							beforePrototypeCreation(beanName); // 不允许发生循环依赖
							try {
								return createBean(beanName, mbd, args);
							}
							finally {
						...
						// 如果是FactoryBean，那么调用FactoryBean#getObject()方法获取真正的Bean的对象
	...
	protected void checkMergedBeanDefinition(RootBeanDefinition mbd, String beanName, @Nullable Object[] args)
		...
		if (mbd.isAbstract()) { // 如果要实例化的Bean是抽象类，那么直接抛出异常
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果bean实例是一个普通bean或者是一个解引用的FactoryBean的时候，直接返回bean实例。
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		// 否则准备从FactoryBean中产生一个bean实例。
		Object object = null;
		if (mbd == null) {
			object = getCachedObjectForFactoryBean(beanName);
		}
		if (object == null) { // 如果从factoryBean的缓存中获取的FactoryBean为null
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {
			if (mbd.isSingleton()) {
				...
				// 如果bean 自定义了destroy method， 注册一个DisposableBean，在
				// ioc容器销毁时， 调用其destroy方法。这里使用了是适配器模式
				// 将不同的类适配成一个DisposableBean
